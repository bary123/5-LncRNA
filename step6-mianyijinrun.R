...
#文件输出路径
outdir = paste0("./Results/06-mianyifenxi/Step1/")
ifelse(dir.exists(outdir),print("文件夹已经存在"),dir.create(outdir,recursive = T))

#小提琴图保存位置
outdirP1= paste0("./Results/06-mianyifenxi/Step2/")
ifelse(dir.exists(outdirP1),print("文件夹已经存在"),dir.create(outdirP1,recursive = T))
...
#加载癌症样本分型
subtype <- read.table(file="./Results/02-ConsensusClusterPlus/UCEC_Clouster.txt",row.names = 1)
table(subtype)
...
samples_meta = data.frame(group = gene_meta[,730])# 获取分组信息
...
SSS <- which(colnames(gene_meta)=='group') #查看分组信息所在的列
tpm = gene_meta[,c(-1,-SSS)]  #去掉基因行名，分组信息，只留基因列
rm(SSS)
one_cell_df = data.frame() 
XXX <- which(colnames(ssGSEA_Score)=='Activated B cell')-1  #查看去掉Activated B cell前面的列：cell in colnames(ssGSEA_Score)[-c(1:732)]
for(t in colnames(tpm)){
  tt = tpm[,t]
  for(cell in colnames(ssGSEA_Score)[-c(1:XXX)]){    
...
outdirP2= paste0(outdirP1,"ssGSEA/")
...
#获取TIMER不同算法里面的细胞分组
tpm1 <- tpm[,1:5]    #为了提高效率，选取几个基因
for(t in colnames(tpm1)){
  tt = tpm1[,t]
  for(cell_m in colnames(immue_eatimate)[-c(1:732)]){
